...
describe('file-system.datasource.ts FileSystemDatasource', () => { 
    ...
    test('should not throw an eror if path exists', () => { 
        new FileSystemDatasource();
        new FileSystemDatasource();
        expect( true ).toBeTruthy();
    });

    test('should return an empty array if the file is empty ', async () => { 
        const LogDatasource = new FileSystemDatasource();
        const emptyLogs = await LogDatasource.getLogs(LogSeverityLevel.low);
        expect( emptyLogs ).toEqual( [] );
    });
    
    test('should throw an error if SeverityLevel does not exist', async () => { 
